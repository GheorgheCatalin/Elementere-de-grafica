...
void Laborator9::Update(float deltaTimeSeconds)
{
...

	{
		...
		doubleTex = true;
		RenderSimpleMesh(meshes["bamboo"], shaders["ShaderLab9"], modelMatrix, mapTextures["bamboo"], mapTextures["earth"]);
	}
}
...
void Laborator9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
	...
	glUniform1i(glGetUniformLocation(shader->program, "doubleTex"), doubleTex);
...
Texture2D* Laborator9::CreateRandomTexture(unsigned int width, unsigned int height)
{
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_MIRRORED_REPEAT); // modul de wrapping pe orizontala
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_MIRRORED_REPEAT); // modul de wrapping pe verticala
