...
void Laborator5::Init()
{
	...
	zFar = 200.0;
	fov = RADIANS(60);
	projType = 0; //1 pentru proiectie perspectiva, 0 pentru ortografica
}
...
void Laborator5::Update(float deltaTimeSeconds)
{
	...
	{
		glm::mat4 modelMatrix = glm::mat4(1);
		modelMatrix = glm::translate(modelMatrix, glm::vec3(-2, 1.5f, -2.0));
...
void Laborator5::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (window->KeyHold(GLFW_KEY_1) && projType == 0) {
		fov = fov + RADIANS(3 * deltaTime);
		projectionMatrix = glm::perspective(fov, window->props.aspectRatio, zNear, zFar);
	}
	if (window->KeyHold(GLFW_KEY_2) && projType == 0) {
		fov = fov - RADIANS(3 * deltaTime);
		projectionMatrix = glm::perspective(fov, window->props.aspectRatio, zNear, zFar);
	} 

	if (window->KeyHold(GLFW_KEY_3) && projType == 1) {
		//if (top <= 20)
		left -= deltaTime;
		right += deltaTime;
		bot -= deltaTime;
		top += deltaTime;
		projectionMatrix = glm::ortho(left, right, bot, top, zNear, zFar);
	}
	if (window->KeyHold(GLFW_KEY_4) && projType == 1) {
		if (top > deltaTime) {
			left += deltaTime;
			right -= deltaTime;
			bot += deltaTime;
